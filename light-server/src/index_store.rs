...
use crate::helper::{read_32, read_u32_be, read_u64_be};
...
impl RocksIndexStore {
    ...
    pub fn tip(&self) -> anyhow::Result<Option<IndexTip>> {
        ...
        Ok(Some(IndexTip {
            height: read_u64_be(&height, "tip_height")?,
            block_hash: BlockHashBytes::from(read_32(&hash, "tip_hash")?),
            last_uid: read_u64_be(&last_uid, "last_uid")?,
        }))
    }
        ...
        for block in blocks {
            for created in &block.created_utxos {
                ...
                let next_count = self
                    .db
                    .get(key)?
                    .map(|bytes| read_u64_be(&bytes, "seen key count"))
                    .transpose()?
                    .unwrap_or(0)
                    .saturating_add(1);
                batch.put(
...
fn encode_spend_lookup(spend: SpendLookup) -> [u8; 21] {
    let mut out = [0u8; 21];
...
fn read_spend_lookup(bytes: &[u8]) -> anyhow::Result<SpendLookup> {
    match bytes.len() {
        21 => Ok(SpendLookup {
            uid: read_u64_be(&bytes[..8], "outpoint uid")?,
            creation_height: read_u64_be(&bytes[8..16], "outpoint creation height")?,
            output_index: read_u32_be(&bytes[16..20], "outpoint storage output index")?,
            flags: bytes[20],
        }),
        20 => Ok(SpendLookup {
            uid: read_u64_be(&bytes[..8], "outpoint uid")?,

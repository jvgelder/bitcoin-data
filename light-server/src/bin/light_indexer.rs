...
async fn run_command(
    ...
    anyhow::ensure!(
        initial_next_height == start_height,
        "resuming real indexing from an existing DB is not implemented yet: next_height={} start_height={}. Delete the SQLite DB and run again, or add state restore from p2tr_utxo_lookup/checkpoint state.",
        initial_next_height,
        start_height
    );
...
fn decode_block_frame(
    ...
    for (tx_index, tx) in block.txdata.iter().enumerate() {
        ...
        for (vout, output) in tx.output.iter().enumerate() {
            ...
            let p2tr_key = p2tr_xonly_output_key(script_bytes);
            let is_p2tr = p2tr_key.is_some();
            let identity_bytes = match p2tr_key {
                Some(key) => key.to_vec(),
                None => script_bytes.to_vec(),
            };
        ...
        txs.push(TxScanInput {
            ...
            inputs,
            outputs,
            // Do not fabricate scan data. Correct BIP352 tx scan points require
            // eligible input pubkeys and, for P2TR key-path inputs, the spent
            // prevout output key. The current raw-block source does not provide
            // complete prevout/undo data, so live indexing omits this legacy field
            // until the 33-byte scan-point schema + prevout source is wired.
...
fn p2tr_xonly_output_key(script_bytes: &[u8]) -> Option<[u8; 32]> {
    // Native P2TR scriptPubKey: OP_1 OP_PUSHBYTES_32 <x-only-output-key>.
...
fn fixture_blocks(
    ...
    for i in 0..count {
        ...
        txs.push(TxScanInput {
            ...
            outputs: vec![
                TxOutputScan {
                    ...
                    is_p2tr: false,
                    is_nums: false,
                    identity_bytes: vec![0x51],
                },
                TxOutputScan {
                    vout: 1,
                    is_p2tr: true,
                    is_nums: false,
                    identity_bytes: vec![height as u8; 32],
                },
            ],
        ...
        let inputs = previous_indexed_outpoint
            .take()
            .map(|previous_output| vec![TxInputScan { previous_output }])
            .unwrap_or_default();
        ...
        txs.push(TxScanInput {
            ...
            outputs: vec![TxOutputScan {
                ...
                is_p2tr: true,
                is_nums: false,
                identity_bytes: vec![height.wrapping_add(1) as u8; 32],
            }],

...
impl TxTweak {
    ...
    pub const fn new(bytes: [u8; 33]) -> Self { Self(bytes) }
    pub const fn as_bytes(&self) -> &[u8; 33] { &self.0 }
    pub const fn into_inner(self) -> [u8; 33] { self.0 }
}

impl From<[u8; 33]> for TxTweak {
    fn from(bytes: [u8; 33]) -> Self { Self(bytes) }
}

impl From<TxTweak> for [u8; 33] {
    fn from(value: TxTweak) -> Self { value.0 }
}
